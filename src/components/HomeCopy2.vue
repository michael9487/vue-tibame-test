...
function animate() {
  ...
  snitches.forEach((s, idx) => {
    ...
    // 我們移除了原本這裡的 Math.cos(s.inclination)，因為我們要用新的方式來做傾斜
    const xBase = rNow * Math.cos(angle);
    const zBase = rNow * Math.sin(angle);

    // 2. 根據深度 (Z) 來計算傾斜高度 (Y)
    // 當 zBase 為負（遠方），tiltY 為正（高）；當 zBase 為正（前方），tiltY 為負（低）
    const tiltY = -zBase * ORBIT_TILT;

    // 3. 保留一點點原本的自然波動感，但幅度減小 (s.yAmp * 0.3)，避免破壞傾斜效果
    // 這裡乘上 u 是為了讓開場動畫時高度也平滑過渡
